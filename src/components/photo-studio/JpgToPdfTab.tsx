  ...
  return (
    <div className="flex flex-col h-full space-y-6 max-w-4xl mx-auto w-full">
      <div className="text-center space-y-2 mb-4">
        ...
        <p className="text-slate-500">Combine multiple images into a single PDF document.</p>
      </div>
      ...
      <div
        ...
        className="min-h-[160px] border-2 border-dashed border-slate-300 rounded-3xl flex flex-col items-center justify-center p-6 text-center cursor-pointer hover:bg-slate-50 transition-colors group"
      >
        ...
        <p className="text-xs text-slate-500 mt-1">or click to add more files</p>
      ...
      {files.length > 0 && !result && (
        <div className="space-y-6">
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 max-h-[300px] overflow-y-auto">
            <h3 className="text-sm font-bold text-slate-700 mb-3 uppercase tracking-wider">{files.length} Images Selected</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {files.map((f, i) => (
                <div key={i} className="relative group bg-white rounded-xl border border-slate-200 overflow-hidden aspect-square flex items-center justify-center">
          ...
          <div className="flex justify-center">
            <button
              ...
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-10 py-3 rounded-full font-semibold transition-all shadow-md active:scale-95 disabled:opacity-50"
            >
      ...
      {result && (
        <div className="bg-green-50 border border-green-200 rounded-3xl p-8 flex flex-col items-center text-center space-y-4">
          ...
          <h3 className="text-xl font-bold text-slate-900">PDF Ready!</h3>
          <p className="text-slate-500">Your images have been successfully converted into a PDF document.</p>
          
          <div className="flex gap-4 mt-4">
            <button
              onClick={() => { setFiles([]); setResult(null); }}
              className="px-6 py-2.5 rounded-full font-medium text-slate-600 bg-white border border-slate-200 hover:bg-slate-50"
            >
              Convert More
            </button>
            <button className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-8 py-2.5 rounded-full font-semibold transition-all shadow-md active:scale-95">
